...
class SoundEventNavSim(Simulator, ABC):
    ...
    def _compute_audiogoal(self):
        ...
        # print("SoundEventNavSim _compute_audiogoal scene: {}".format(self._current_scene))
        # print("SoundEventNavSim _compute_audiogoal step_idx: {}".format(step_idx))
        ...
        if (
            ...
            audiogoal = np.zeros((4, sr))
        else:
            if self.config.USE_RENDERED_OBSERVATIONS:
                ambisonic_rir_file = os.path.join(self.ambisonic_rir_dir, 
                ...
                try:
                    sampling_freq, ambisonic_rir = wavfile.read(ambisonic_rir_file)
                except ValueError:
                    logging.warning("{} file is not readable".format(ambisonic_rir_file))
                    ambisonic_rir = np.zeros((sr, 4))
                if len(ambisonic_rir) == 0:
                    logging.warning("Empty RIR file at {}".format(ambisonic_rir_file))
                    ambisonic_rir = np.zeros((sr, 4))
            else:
                ambisonic_rir = np.transpose(np.array(self._sim.get_sensor_observations()["audio_sensor"]))

            # ambisonic_rir = np.transpose(np.array(self._sim.get_sensor_observations()["audio_sensor"]))
            rir_len = ambisonic_rir.shape[0]
            ...
            audiogoal = np.array([
                fftconvolve(goal_sound, ambisonic_rir[:, channel], mode='valid')
                for channel in range(ambisonic_rir.shape[-1])
            ])
