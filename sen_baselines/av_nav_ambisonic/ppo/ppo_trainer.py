...
from sen_baselines.common.utils import (
    batch_obs,
    generate_video,
    linear_decay,
    plot_top_down_map,
...
from sen_baselines.av_nav_ambisonic.ppo.policy import AudioNavBaselinePolicy
from sen_baselines.av_nav.ppo.ppo import PPO
...
class PPOTrainer(BaseRLTrainer):
    ...
    def _collect_rollout_step(
            self, rollouts, current_episode_reward, current_episode_step, episode_rewards, episode_spls, episode_counts, episode_steps,
    ):
        pth_time = 0.0
        env_time = 0.0
    ...
    def train(self) -> None:
        ...
        # 返回的数据类型是habitat.VectorEnv
        # 这是一个可向量化的环境类, 可以创建多个进程, 其中每个进程运行自己的环境
        # 是用于并行化训练和评估的主要类
